    ...
    // if key is found in the array, return the location, otherwise return -1  
{
    ...
    if (x[idx] == key) return idx;
    else if (x[idx] > key) return binary_search(lo, idx - 1, key);
    else if (x[idx] < key) return binary_search(idx + 1, hi, key);
    ...
    // if key is found in the array, return the location, otherwise return -1  
{
    ...
    while (hi >= lo) {
        ...
        if (key == x[idx]) return idx;
        else if (key > x[idx]) lo = idx + 1;
        else hi = idx - 1;
    }
    return -1 ;
}
